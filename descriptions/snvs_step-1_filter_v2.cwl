#!/usr/bin/env cwl-runner

cwlVersion: v1.0

class: Workflow

requirements:
  MultipleInputFeatureRequirement: {}

inputs:
  - id: genome_reference_fasta
    type: File
    secondaryFiles:
      - ^.dict
      - .fai
    doc: Genome reference in FASTA format with the corresponding index files

  - id: input_files_TNhaplotyper2_vcf_gz
    type:
      -
        items: File
        type: array
    secondaryFiles:
      - .tbi
    doc: List of input VCF files generated by TNhaplotyper2. Compressed with the corresponding index file

  - id: input_files_core_ids_TNhaplotyper2
    type:
      -
        items: string
        type: array
    doc: Core identifiers for VCF files generated by TNhaplotyper2 (1:1 match) 

  - id: input_files_Strelka2_vcf_gz
    type:
      -
        items: File
        type: array
    secondaryFiles:
      - .tbi
    doc: List of input VCF files generated by Strelka2. Compressed with the corresponding index file

  - id: input_files_core_ids_Strelka2
    type:
      -
        items: string
        type: array
    doc: Core identifiers for VCF files generated by Strelka2 (1:1 match) 

  - id: input_files_longcallD_vcf_gz
    default: []
    type:
      -
        items: File
        type: array
    secondaryFiles:
      - .tbi
    doc: List of input VCF files generated by longcallD. Compressed with the corresponding index file

  - id: input_files_core_ids_longcallD
    default: []
    type:
      -
        items: string
        type: array
    doc: Core identifiers for VCF files generated by longcallD (1:1 match) 

  - id: input_files_RUFUS_vcf_gz
    type:
      -
        items: File
        type: array
    secondaryFiles:
      - .tbi
    doc: List of input VCF files generated by RUFUS. Compressed with the corresponding index file

  - id: input_files_core_ids_RUFUS
    type:
      -
        items: string
        type: array
    doc: Core identifiers for VCF files generated by RUFUS (1:1 match) 

  - id: germline_input_file_vcf_gz
    type: File
    secondaryFiles:
      - .tbi
    doc: Germline SNV calls input VCF (bgzipped) with .tbi index, do not need to be filtered ahead of time

  - id: input_files_bed
    type:
      -
        items: File
        type: array
    doc: List of BED files with regions to exclude from the VCF

  - id: panel_of_normals_fasta
    type: File
    secondaryFiles:
      - .fai
    doc: Panel of normals in FASTA format with the corresponding index file

  - id: regions_list_txt
    type: File
    doc: Regions list file (one region per line, e.g. chr1:1-1000000) for VEP

  - id: vep_database_archive
    type: File
    doc: Compressed VEP database archive (tar.gz)

  - id: gnomad_vcf_gz
    type: File
    secondaryFiles:
      - .tbi
    doc: Compressed gnomAD VCF file with corresponding index file

  - id: tag_filters
    type: string[]
    doc: One or more tag filters. |
         Format "name/value/operator/type/logic[/field=sep][/entry=sep]"

  - id: sample_name
    type: string
    doc: Name of the sample. This will be added to the merged VCF header

  - id: window_size
    type: int
    default: 50
    doc: Window size in bp [50]

outputs:
  output_snvs_vcf_gz:
    type: File
    outputSource: split_snvs_indels/output_snvs_vcf_gz

  output_indels_vcf_gz:
    type: File
    outputSource: split_snvs_indels/output_indels_vcf_gz

steps:
  merge_callers_v2:
    run: merge_callers_v2.cwl
    in:
      input_files_TNhaplotyper2_vcf_gz:
        source: input_files_TNhaplotyper2_vcf_gz
      input_files_core_ids_TNhaplotyper2:
        source: input_files_core_ids_TNhaplotyper2
      input_files_Strelka2_vcf_gz:
        source: input_files_Strelka2_vcf_gz
      input_files_core_ids_Strelka2:
        source: input_files_core_ids_Strelka2
      input_files_longcallD_vcf_gz:
        source: input_files_longcallD_vcf_gz
      input_files_core_ids_longcallD:
        source: input_files_core_ids_longcallD
      input_files_RUFUS_vcf_gz:
        source: input_files_RUFUS_vcf_gz
      input_files_core_ids_RUFUS:
        source: input_files_core_ids_RUFUS
      sample_name:
        source: sample_name
    out:
      - output_file_vcf_gz

  filter_germline_variants:
    run: filter_germline_variants.cwl
    in:
      input_file_vcf_gz:
        source: merge_callers_v2/output_file_vcf_gz
      germline_input_file_vcf_gz:
        source: germline_input_file_vcf_gz
    out:
      - output_file_vcf_gz

  filter_clustered_variants_v2:
    run: filter_clustered_variants_v2.cwl
    in:
      input_file_vcf_gz:
        source: filter_germline_variants/output_file_vcf_gz
      window_size:
        source: window_size
    out:
      - output_file_vcf_gz

  bcftools_regions:
    run: bcftools_regions.cwl
    in:
      input_file_vcf_gz:
        source: filter_clustered_variants_v2/output_file_vcf_gz
      input_files_bed:
        source: input_files_bed
    out:
      - output_file_vcf_gz

  filter_panel_errors:
    run: filter_panel_errors.cwl
    in:
      input_file_vcf_gz:
        source: bcftools_regions/output_file_vcf_gz
      panel_of_normals_fasta:
        source: panel_of_normals_fasta
    out:
      - output_file_vcf_gz

  vep_parallel:
    run: vep-parallel.cwl
    in:
      input_file_vcf_gz:
        source: filter_panel_errors/output_file_vcf_gz
      regions_list_txt:
        source: regions_list_txt
      vep_database_archive:
        source: vep_database_archive
      gnomad_vcf_gz:
        source: gnomad_vcf_gz
      genome_reference_fasta:
        source: genome_reference_fasta
    out:
      - output_file_vcf_gz

  granite_filterByTag:
    run: granite_filterByTag.cwl
    in:
      input_file_vcf_gz:
        source: vep_parallel/output_file_vcf_gz
      tag_filters:
        source: tag_filters
    out:
      - output_file_vcf

  compress_index_vcf:
    run: compress_index_vcf.cwl
    in:
      input_file_vcf:
        source: granite_filterByTag/output_file_vcf
    out:
      - output_file_vcf_gz

  split_snvs_indels:
    run: split_snvs_indels.cwl
    in:
      input_file_vcf_gz:
        source: compress_index_vcf/output_file_vcf_gz
    out:
      - output_snvs_vcf_gz
      - output_indels_vcf_gz

doc: |
  Filters a VCF file to retain high-confidence variants. |
  Step-1 filters: Merge variants from callers (need pre-filtered PASS calls, normalized and atomized) (includes core-specific and merged calling); |
  remove germline and clustered variants; remove difficult genomic regions; |
  Brain Somatic Mosaicism Network (BSMN) filter; run VEP and filter by allele frequency. |
  Splits the filtered variants into separate SNV and indel VCF files
