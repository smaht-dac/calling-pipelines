...
#     1) parse_minipileup_sr_only_v2.py -> <prefix>.CrossTissue.vcf.gz (+tabix)
#     2) set_filter_vcf_v2.py           -> <prefix>.final.vcf.gz      (+tabix)
#################################################################################
...
Usage: parse_CrossTissue_minipileup_result.sh -i orig.vcf.gz --mp mp.vcf.gz --tissue TISSUE [-o prefix]
  ...
  --tissue          Tissue label passed to parse_minipileup_sr_only_v2.py (required)
...
Notes:
  - Assumes these are on PATH: tabix, python3,
      parse_minipileup_sr_only_v2.py, set_filter_vcf_v2.py
EOF
  exit 1
}
...
command -v parse_minipileup_sr_only_v2.py >/dev/null 2>&1 || { echo "Error: parse_minipileup_sr_only_v2.py not found in PATH"; exit 1; }
command -v set_filter_vcf_v2.py >/dev/null 2>&1          || { echo "Error: set_filter_vcf_v2.py not found in PATH"; exit 1; }
...
parse_minipileup_sr_only_v2.py \
  --tissue "$TISSUE" \
  --orig_vcf "$ORIG_VCF" \
  --mp_vcf "$MP_VCF" \
  --out "$CROSSTISSUE_VCF_GZ" \
  || { echo "Error: parse_minipileup_sr_only_v2.py failed"; exit 1; }
...
set_filter_vcf_v2.py \
  -i "$CROSSTISSUE_VCF_GZ" \
  -o "$FINAL_VCF_GZ" \
  || { echo "Error: set_filter_vcf_v2.py failed"; exit 1; }
